...
SELECT * FROM users
WHERE email = $1;
`, [email]).then(res => res ? res.rows[0] : null)
    .catch(err => {
      console.log(err)
    });
};
...
SELECT * FROM users
WHERE id = $1;
`, [id]).then(res => res ? res.rows[0] : null)
    .catch(err => {
      console.log(err)
    });
};
...
const addUser =  function(user) {
  return pool.query(`
    ...
    VALUES($1, $2, $3)
    RETURNING *;
  `,[user.name, user.email, user.password]).then(res => res.rows[0]).catch(err => {
    console.log(err);
  });
};
...
const getAllProperties = function(options, limit = 10) {
  ...
  } else if (!options.city && !options.owner_id && options.minimum_price_per_night) {
    queryParams.push(options.minimum_price_per_night);
  ...
  } else if (!options.city && !options.owner_id && !options.minimum_price_per_night && options.maximum_price_per_night) {
    queryParams.push(options.maximum_price_per_night);
    queryString += `WHERE cost_per_night <= $${queryParams.length} `;
  }
